...
def get_cli_args():
    parser = ArgumentParser(
        "Runs localization filters (EKF or PF) on generated, simulation data."
    )
    parser.add_argument(
        "-i",
        "--input-data-file",
    ...
    parser.add_argument(
        "--dt", type=float, action="store", help="Time step (in seconds).", default=0.1
    )
    ...
    parser.add_argument(
        "--global_localization",
        action="store_true",
        help="Task E, Global localization enabled, only for PF.",
    )
...
def validate_cli_args(args):
    if args.input_data_file and not os.path.exists(args.input_data_file):
        raise OSError(
            "The input data file {} does not exist.".format(args.input_data_file)
        )

    if not args.input_data_file and not args.num_steps:
        raise RuntimeError(
            "Neither `--input-data-file` nor `--num-steps` were present in the arguments."
        )

    if args.filter_name != "pf" and args.global_localization:
        warn(
            "Global localization is only supported for the particle filter. Ignoring the flag."
        )

    if not args.animate:
        if args.show_trajectory:
            warn(
                "Since animation for the simulation was not enabled, ignoring `--show-trajectory`."
            )
        if args.show_particles:
            warn(
                "Since animation for the simulation was not enabled, ignoring `--show-particles`."
            )

    if args.show_particles and args.filter_name != "pf":
        warn(
            "Since the simulation is not running the particle filter, ignoring `--show-particles`."
        )

    if args.show_particles and args.output_dir:
        warn(
            "Since `--output-dir` is specified, ignoring `--show-particles` to generate just one trajectory."
        )
...
def main():
    ...
    if args.input_data_file:
        ...
        data = generate_input_data(
            initial_state.mu.T, args.num_steps, alphas, bearing_std, args.dt
        )
    else:
        raise RuntimeError("")
    ...
    one_trajectory_per_particle = (
        True if show_particles and not store_sim_data else False
    )
    ...
    if args.filter_name == "ekf":
        ...
        localization_filter = PF(
            initial_state,
            alphas,
            bearing_std,
            args.num_particles,
            args.global_localization,
        )
    ...
    if update_mean_trajectory:
        if one_trajectory_per_particle:
            mean_trajectory = np.zeros(
                (data.num_steps, localization_filter.state_dim, args.num_particles)
            )
        else:
    ...
    if store_sim_data:
        ...
        sim_trajectory.covariance = np.zeros(
            (
    ...
    if write_movie:
        ...
        movie_fps = min(args.movie_fps, float(1.0 / args.plot_pause_len))
    ...
    ) if write_movie else get_dummy_context_mgr():
        for t in range(data.num_steps):
            ...
            if show_trajectory:
                if len(sim_trajectory.mean.shape) > 2:
                    ...
                    plt.plot(
                        sim_trajectory.mean[0:t, 0], sim_trajectory.mean[0:t, 1], "blue"
                    )
