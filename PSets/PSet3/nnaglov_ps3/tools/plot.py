...
def plot_robot(state, radius=15.0):
    """
    ...
    robot = plt.Circle(
        state[:-1], radius, edgecolor="black", facecolor="cyan", alpha=0.25
    )
    orientation_line = np.array(
        [
            [state[0], state[0] + (np.cos(state[2]) * (radius * 1.5))],
...
def plot_field(field_map, detected_landmarks):
    ...
    plt.axis(
        (
            -margin,
            field_map.complete_size_x + margin,
            -margin,
...
def plot_observations(pose, noise_free_observations, noisy_observations):
    ...
    for k in range(M):
        ...
        plt.plot(
            [pose[0], pose[0] + noisy_range * np.cos(pose[2] + noisy_bearing)],
        ...
        plt.plot(
            [pose[0], pose[0] + nf_range * np.cos(pose[2] + nf_bearing)],
