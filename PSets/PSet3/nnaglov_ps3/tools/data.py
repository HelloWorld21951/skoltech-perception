...
def sense_landmarks(state, field_map, max_observations):
    ...
    noise_free_observation_tuples = [
        (x[0], np.abs(x[1]), int(x[2])) for x in noise_free_observations_list
    ]
    ...
    c1 = noise_free_observations[:, 1] > -np.pi / 2.0
    c2 = noise_free_observations[:, 1] < np.pi / 2.0
...
def generate_data(
    ...
    filter_data = SlamInputData(
        np.zeros((data_length, motion_dim)),
        np.empty((data_length, max_obs_per_time_step, observation_dim)),
    )
    debug_data = SlamDebugData(
    ...
    for i in range(1, data_length):
        ...
        noise_free_observations = sense_landmarks(
            debug_data.real_robot_path[i], field_map, max_obs_per_time_step
        )
